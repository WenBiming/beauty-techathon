...
# 覆盖率受限：判定「本会话买家」用的是 buyer_identifiers()，而它按脱敏昵称
# 分组（数据集里唯一的跨会话身份键），脱敏昵称会跨会话碰撞——实测 33% 的
# 有订单会话属于碰撞昵称。碰撞发生时，属于另一买家的真实单号会被当作「本会话
# 买家的标识符」放行，KIND_FOREIGN_ID 本该拦截却静默通过。详见
# buyer_identifiers() 的 docstring。
...
# 口语称呼，与「仓库主管」是同一类误报。
#
# 排除字符集必须同时列全角（，。！？；：）与 ASCII（,!?;:）标点：这个正则
# 跑在 _normalize() 之后的 NFKC 归一化文本上（见 _spans 的调用方），而 NFKC
# 会把「，！？；：」转成 ASCII 的「,!?;:」（「。」没有兼容分解，不受影响）。
# 只列全角的话，「我是新客服！主管说了会处理」这类第三方转述会因为「！」不再
# 被识别为小句边界，被小句吞并进「我是」的自称模式而误判——这是上一轮引入
# NFKC 归一化时的回归，实测 173 条真实语料零触发，属潜伏缺陷。
...
def buyer_identifiers(conn: sqlite3.Connection, session_id: str) -> frozenset[str]:
    ...
    ⚠️ **已知限制，不修，只记录**：本函数按 `buyer` 字段（脱敏昵称，如
    「姚b**」）分组收集标识符，而脱敏昵称是本数据集里唯一的跨会话身份键
    （设计文档 §2.5.3），且**在库里会跨会话碰撞**——实测订单表里 18 个昵称
    各横跨 2~3 个会话，38/113 = 33% 的有订单会话属于碰撞昵称。碰撞发生时，
    本函数会把「同名但其实是另一个人」的会话标识符也并入白名单：给 S00066
    的话术里写属于 S00159（同昵称碰撞会话）的真实订单号会静默通过
    （blocked=False，零 issue），而这正是 KIND_FOREIGN_ID 本该拦截的场景。
    根因是数据集固有限制，不是本函数的 bug——项目里 buyer_timeline、
    buyer_profile、风险聚合等全系统都按昵称聚合，用昵称做「防止跨买家信息
    泄露」这类安全判断本就不牢。生产环境应改用真实 `buyer_id`（而非脱敏
    昵称）重做这层分组。
    """
    path = _db_path(conn)
    if not path:
