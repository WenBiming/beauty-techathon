...
# ↑ 这 4 条与 agent/compliance.py 的正则一一对应（_ID_PATTERNS / _HONORIFIC /
# _PROMISE_HARD）：提示词是第一道防线，compliance 是第二道确定性拦截。
# **改动任一处必须同步另一处**，否则会出现「提示词禁了但正则不查」的分叉——
# 实测发生过：提示词点名禁「每 4 小时」，正则里却没有周期性时限。
